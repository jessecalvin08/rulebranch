...
Do not create or share another API key. Sandbox access is now granted (`spawn=true` on September 19, 2026), but the SDK warned the token expires within the hour, so confirm the credential is valid before a paid run. One real but partial run is recorded; the next goal is a complete observe-and-enforce comparison. Approve the current sample policy (23/23) in the local dashboard and run the command it gives you; the measured result appears in the dashboard's Sandbox evidence section.
