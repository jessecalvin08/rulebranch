...
- Be truthful in docs, UI, and copy: live Nemotron policy generation is verified; a live Sandbox run is NOT (project returned 403 / `spawn=false`). The dashboard comparison is a scripted simulation, not measured agent behavior. Don't add measured claims without real evidence in ignored `reports/`.
